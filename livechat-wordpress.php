<?php
/*
 * Plugin Name:	LIVEChat for Wordpress & Rocket.Chat
...
final class RocketChat_LIVEChat
{
	...
	public function __construct()
	{
		...
		if ($settings = get_option('rocketchat-livechat-settings')) {
			$this->settings = get_option('rocketchat-livechat-settings');
		...
		$this->setting_liveChat = get_option('rocketchat_livechat_tag-settings');


		$this->setup();


		add_action('wp_dashboard_setup', array($this, '_rocketchat-livechat_dashboard_widget_setup'));
	}

	/*
	 * Display the rocketchat-livechat validation
	 */
	...
	private function _load_settings()
	{
		...
		$this->settings = get_option('rocketchat-livechat-settings', false);

		$this->livechat_tag_settings = get_option('rocketchat_livechat_livechat_tag-settings', false);

		//Save Default setting
		$this->default = array(
			'form_title'     => __('Rocket.Chat LIVChat Form Submission', 'rocketchat-livechat'),
			'fullname'       => __('Name', 'rocketchat-livechat'),
			'email'          => __('Email', 'rocketchat-livechat'),
			'tickettype'     => __('Select Ticket Type', 'rocketchat-livechat'),
			'ticketpriority' => __('Select Ticket Priority', 'rocketchat-livechat'),
			'department'     => __('Select Department', 'rocketchat-livechat'),
			'subject'        => __('Subject', 'rocketchat-livechat'),
			'contents'       => __('Description', 'rocketchat-livechat'),
	 ...
	 * array which is kept under the 'rocketchat-livechat-settings' key inside
	...
	public function rocketchat_admin_init()
	{
		...
		register_setting('rocketchat-livechat-settings', 'rocketchat-livechat-settings', '');
		add_settings_section('api_setting_form', __('Access Details', 'rocketchat-livechat'), array(&$this, '_settings_section_api_setting_form'), 'rocketchat-livechat-settings');
		add_settings_field('rocketchat_livechat_url', __('Your Rocket.Chat API URL', 'rocketchat-livechat'), array(&$this, '_settings_field_rocketchat_livechat_url'), 'rocketchat-livechat-settings', 'api_setting_form');
		add_settings_field('rocketchat-livechat_key', __('API Key', 'rocketchat-livechat'), array(&$this, '_settings_field_api_key'), 'rocketchat-livechat-settings', 'api_setting_form');
		add_settings_field('rocketchat-livechat_secret', __('API Secret', 'rocketchat-livechat'), array(&$this, '_settings_api_secret'), 'rocketchat-livechat-settings', 'api_setting_form');
		add_settings_section('api_setting_form', __('Access Details', 'rocketchat-livechat'), array(&$this, '_settings_section_api_setting_form'), 'rocketchat-livechat-settings');


		//Rocket.Chat LIVEChat Settings
		register_setting('rocketchat-livechat_tag-settings', 'rocketchat-livechat_tag-settings', '');
		add_settings_section('rocketchat_livechat_tag_form', __('Live Chat tag generator', 'rocketchat-livechat'), array(
			&$this, 'rocketchat_livechat_tag_settings_description'
		), 'rocketchat-livechat_tag-settings');
		add_settings_field('rocketchat-livechat_tag', __('Paste the tag generated by Rocket.Chat LIVEChat here: ', 'rocketchat-livechat'), array(
			&$this, '_livechat_tag_textarea'
		), 'rocketchat-livechat_tag-settings', 'rocketchat_livechat_tag_form');
		add_settings_section('rocketchat_livechat_note', __('Note :', 'rocketchat-livechat'), array(&$this, '_rocketchat_livechat_note'), 'rocketchat-livechat_tag-settings');
	...
	{
		add_menu_page('Rocket.Chat LIVEChat for Wordpress', 'Rocket.Chat LIVEChat', 'manage_options', 'liveitpros-support', array(
			&$this, '_admin_menu_contents'
		), plugins_url('rocketchat-livechat/images/small_esupport.gif'), 99);
		add_submenu_page('liveitpros-support', 'Rocket.Chat LIVEChat Settings', __('Rocket.Chat LIVEChat Settings', 'support'), 'manage_options', 'liveitpros-support', array(&$this, '_admin_menu_contents'));
		add_submenu_page('liveitpros-support', 'Rocket.Chat LIVEChat Tag', __('Rocket.Chat LIVEChat Tag', 'support'), 'manage_options', 'liveitpros-livechat_tag-settings', array(
	...
	public function _admin_menu_contents()
	{
		...
		if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'convert_comments' || $_REQUEST['c']) {
			...
			<div class="wrap">
				...
				<h2><?php _e('Rocket.Chat LIVEChat for WordPress Settings', 'rocketchat-livechat'); ?></h2>

				<form method="post" action="options.php">
					<input type="hidden" name="rocketchat-livechat-settings[saved]"/>
					<?php wp_nonce_field('update-options'); ?>
					<?php settings_fields('rocketchat-livechat-settings'); ?>
					<?php do_settings_sections('rocketchat-livechat-settings'); ?>
					<p class="submit" align="left">
						<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Details', 'rocketchat-livechat'); ?>"/>
					</p>
				</form>
			</div>
		<?php
	...
	{
		_e('Please enter your Rocket.Chat LIVEChat API Key, Secret Key, Rocket.Chat LIVEChat URL for further connection !', 'rocketchat-livechat');
	}
	...
	public function _settings_field_rocketchat_livechat_url()
	{
		...
		?>
		<input type="text" name="rocketchat-livechat-settings[rocketchat_livechat_url]" id="rocketchat-livechat-settings[rocketchat_livechat_url]" size="60" value="<?php echo $rocketchat_livechat_url; ?>"/> <?php
	}
	...
	public function _settings_field_api_key()
	{
		...
		?>
		<input type="text" name="rocketchat-livechat-settings[rocketchat_livechat_key]" id="rocketchat-livechat-settings[rocketchat_livechat_key]" size="60" value="<?php echo $rocketchat_livechat_key; ?>"/> <?php
	}
	...
	public function _settings_api_secret()
	{
		...
		?> <input type="text" name="rocketchat-livechat-settings[rocketchat_livechat_secret]" id="rocketchat-livechat-settings[rocketchat_livechat_secret]" size="60" value="<?php echo $rocketchat_livechat_secret; ?>"/> <?php
	}
	...
	public function _settings_field_form_title()
	{
		...
		?>
		<input type="text" size="40" name="rocketchat-livechat-settings[form_title]" value="<?php echo $form_value; ?>" placeholder="Rocket.Chat LIVEChat Form"/>
	<?php
	}
	...
	public function _settings_field_fullname()
	{
		...
		?>
		<input type="text" size="40" name="rocketchat-livechat-settings[fullname]" value="<?php echo $fullname; ?>" placeholder="Please enter your fullname"/>
	<?php
	}
	...
	public function _settings_field_email()
	{
		...
		?>
		<input type="text" size="40" name="rocketchat-livechat-settings[email]" value="<?php echo $email; ?>" placeholder="Your Email Address"/>
	<?php
	}

	/* Rocket.Chat LIVEChat Dashboard Widget
	 * It displays a ticket view list on the frontend of rocketchat-livechat widget setup
	 */
	public function _rocketchat_livechat_dashboard_widget_setup()
	{
		if ($this->settings) {
			wp_add_dashboard_widget('livechat-dashboard-viewticket-widget', __('Rocket.Chat LIVEChat', 'rocketchat-livechat'), array(&$this, '_admin_view_tickets_bar'));
		}
	}
	...
	public function _livechat_form_contents()
	{
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				<?php wp_nonce_field('update-options'); ?>
				<?php settings_fields('rocketchat-livechat_tag-settings'); ?>
				<?php do_settings_sections('rocketchat-livechat_tag-settings'); ?>
				<p class="submit">
					<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Tag', 'rocketchat-livechat'); ?>"/>
				</p>
			</form>
		</div>
	<?php
	...
	public function _livechat_tag_textarea()
	{
		...
		?>
		<textarea rows="10" cols="100" name="rocketchat-livechat_tag-settings[rocketchat_livechat_tag]" id="rocketchat-livechat_tag-settings[rocketchat_livechat_tag]"><?php echo $rocketchat_livechat_tag; ?></textarea>
	<?php
	}
	...
	{
		_e('The Rocket.Chat LIVEChat places a convenient chat tab on your pages that allow your visitors to initiate a chat !', 'rocketchat-livechat');
	}
	...
	{
		_e('After setting your options, include the following template snippet anywhere in your theme to display the Rocket.Chat LIVEChat Tag Icon: <p><code><</code><code>?php if(function_exists(\'the_rocketchat_liveChatTag\')) the_rocketchat_liveChatTag(); ?</code><code>></code></p>', 'rocketchat-livechat');
	}
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script('my-script', plugins_url('/js/rocketchat-livechat.js', __FILE__), array('jquery'));
...
{
	global $rocketchat-livechat;

	if ($rocketchat-livechat) {
		$rocketchat-livechat->rocketchat_livechat_tag_code();
	}
} 
?>